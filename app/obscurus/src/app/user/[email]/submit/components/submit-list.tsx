...
export default function SubmitList({
  ...
  const handleClick = (item: Requests) => {
    if (!upload) {
      router.push(`${pathname}/../${item.requestId}`);
  ...
  const tabsContent = statuses.map((status) => {
    const filteredRequests = sortedRequests?.filter((request) => {
      ...
      const matchesSearch =
        ...
        return status === "all" ? matchesSearch : matchesStatus && matchesSearch;
    });
  ...
  return requests && submissions ? (
    <Tabs defaultValue="all" className="h-screen overflow-scroll">
